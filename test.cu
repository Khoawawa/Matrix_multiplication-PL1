// strassen_benchmark.cu
// Compile: nvcc -O3 -arch=sm_70 strassen_benchmark.cu -o benchmark
// Run: ./benchmark
...
void runBenchmark(int N) {
    ...
    // 3. Run Naive GPU (làm Reference vì CPU quá chậm với N=4000)
    {
        double *dA, *dB, *dC;
...
int main(int argc, char* argv[]) {
    ...
    // Nếu có tham số dòng lệnh thì chạy size đó, không thì chạy bộ test mặc định
    if (argc > 1) {
        int n = atoi(argv[1]);
        runBenchmark(n);
    } else {
        vector<int> tests = {1024, 2048, 4000, 4096}; // Đã thêm 4000
        for (int n : tests) {
            runBenchmark(n);
        }
    }
